...
export class CustomerComponent implements OnInit {
  ...
  callFunction(e: { dishname: any; price: any }) {
    ...
    try {
      ...
      const menuresp = this.sharedser.fetchmenu().subscribe((resp) => {
        ...
        for (let index = 0; index < this.menudata1.length; index++) {
          ...
          try {
            this.menudata1[0].finalPrice =
              this.sharedser.cartdata[index].finalPrice;
        ...
        for (let j = 0; j < this.sharedser.cartdata.length; j++) {
          const index1 = this.menudata1.findIndex(
            (x) => x.dishname === this.sharedser.cartdata[j].dishname
          );
      ...
      const menuresp = this.sharedser.fetchmenu().subscribe((resp) => {
        ...
        for (let j = 0; j < this.sharedser.cartdata.length; j++) {
          try {
            ...
            this.menudata1[index1].quantity =
              this.sharedser.cartdata[j].quantity;
          } catch (err) {
            this.menudata1[0].finalPrice = 0;
  ...
  changeQty(pid: number, qty: number) {
    ...
    if (this.sharedser.cartdata.length > 0) {
      try {
        const index = this.sharedser.cartdata.findIndex(
          (x) => x.dishname === this.menudata1[pid].dishname
        );
        const quantity =
          Number(this.sharedser.cartdata[index].quantity) + Number(qty);
