...
export class LandingComponent implements OnInit {
  ...
  getrouting() {
    try {
      this._router.resetConfig([
        {
          path: '',
          redirectTo: '/' + this.jsonData[0].landing,
          pathMatch: 'full',
        },
  ...
  ngOnInit() {
    // this.jsonData = this.sharedsrv.sharedConfig;
  ...
  login() {
    this._router.navigateByUrl('/' + this.config.getConfig('login'));
  }
  register() {
    this._router.navigateByUrl('/' + this.config.getConfig('register'));
  }
}
